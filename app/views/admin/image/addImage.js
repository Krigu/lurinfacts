(function() {
  ...
  angular
    .module("lurinfacts")
    .controller("AddImageCtrl", function(
      $scope,
      ImageResizeService,
      ...
      vm.updateLocationByCoords = function(coords) {
        GeoLocationService.getPositionByCoords(coords.latitude, coords.longitude).then(
          function(location) {
            vm.newPoint.location = location;
          },
          function(error) {
            NotificationService.error(
              "lurin hacked the application, something went druing address lookup."
            );
      ...
      vm.getPositionByAddress = function() {
        GeoLocationService.getPositionByAddress(vm.newPoint.location.address).then(
          function(location) {
            vm.newPoint.location = location;
            var coords = {
      ...
      vm.SavePicture = function() {
        ...
        ImageLocationService.saveLocation(vm.newPoint, vm.imageSource, vm.imageThumb).then(
          function(data) {
          ...
          function(error) {
            ...
            NotificationService.error(
              "lurin hacked the application, something went wrong during save."
            );
          }
        );
      };
